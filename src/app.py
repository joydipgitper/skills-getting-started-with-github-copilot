...
activities = {
         "Chess Club": {
            ...
            "Drama Club": {
                "description": "Theater, acting, and stage performance workshops",
...
def signup_for_activity(activity_name: str, email: str):
    ...
    if any((p or "").strip().lower() == normalized for p in participants):
        raise HTTPException(status_code=400, detail="Student is already signed up")

    # Validate capacity
    if len(participants) >= activity.get("max_participants", float("inf")):
...
def unregister_from_activity(activity_name: str, email: str):
    ...
    for i, p in enumerate(participants):
        if (p or "").strip().lower() == normalized:
            participants.pop(i)
            activity["participants"] = participants
